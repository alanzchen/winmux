...
private let configParser: [String: any ParserProtocol<Config>] = [
    ...
    "middle-click-closes-windows": Parser(\.middleClickClosesWindows, parseBool),
    ...
    "open-new-windows-in-new-workspace": Parser(\.openNewWindowsInNewWorkspace) { parseBool($0, $1).map { Optional($0) } },
