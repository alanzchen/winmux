...
enum SettingsCatalog {
    ...
    static let fields: [SettingsField] = {
        ...
        var result: [SettingsField] = [
            ...
            int(.sidebarAppearance, "width", "Expanded width", "Panel width in points for window details and search. With Always expanded on, you can also drag the panel's inner edge.", section: sidebar, range: workspaceSidebarResizableWidthRange, path: \.workspaceSidebar.width),
            ...
            bool(.newWindows, "open-new-windows-in-new-workspace", "Open new windows in a new workspace", "Give each window you open its own empty workspace in the current project. Dialogs, restored windows, and windows placed by on-window-detected rules stay where they are. Takes precedence over adding to the current tab group.", path: \.openNewWindowsInNewWorkspace),
